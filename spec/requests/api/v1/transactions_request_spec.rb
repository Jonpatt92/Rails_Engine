...
describe "Invoice Items API" do
  before(:each) do
    ...
    @transaction_1    = create(:transaction, credit_card_number: 2222, result: "failed", invoice: @invoice )
    @transaction_2    = create(:transaction, credit_card_number: 3333, result: "unpaid", invoice: @invoice )
    @transaction_3    = create(:transaction, credit_card_number: 4444, result: "wheres my money", invoice: @invoice )
  ...
  it "Can show a list of all transactions" do
    ...
    expect(transactions["data"][0]["attributes"]["credit_card_number"]).to eq(@transaction_1.credit_card_number.to_s)
    ...
    expect(transactions["data"][1]["attributes"]["credit_card_number"]).to eq(@transaction_2.credit_card_number.to_s)
    ...
    expect(transactions["data"][2]["attributes"]["credit_card_number"]).to eq(@transaction_3.credit_card_number.to_s)
  ...
  it "Can get one transaction by its id" do
    ...
    expect(transaction["data"]["attributes"]["credit_card_number"]).to eq(@transaction_1.credit_card_number.to_s)
  ...
  it "Can show the invoice that a specific transaction belongs to" do
    ...
    expect(invoice_json["data"]["attributes"]["payment_status"]).to eq("unpaid")
